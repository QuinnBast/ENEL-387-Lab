...
int main(){
	...
	while(1){
		...
		for(int i=0; i<8; i++){
			...
			buttonState[i] = readSwitch(i);
		}
	
		if(buttonState[5] == 1 && previousState[5] == 0){
			startStop();
		}
		if(buttonState[6] == 1 && previousState[6] == 0){
			nextTrack();
		}
		if(buttonState[7] == 1 && previousState[7] == 0){
			prevTrack();
		}
		*/
